...
def mainMenu():
    ...
    menuTitle = menu_font.render("Menu [m]", True, black)
    frame.blit(menuTitle, (menux+120,menuy+20))

    # play button
    playButton = button_font.render("Play                            [p]", True, white)
    ...
    # pouch button
    pouchButton = button_font.render('View Treasures        [t]', True, white)
    ...
    # high score button
    highScoreButton = button_font.render("High Score                [s]", True, white)
    ...
    # quit button
    quitButton = button_font.render("Quit                            [q]", True, white)
...
def saveHighScore(name):
    ## MAKE SO CAN ONLY SAVE HS IF SCORE > ENEMY SCORE
    ## MAKE AUTO GAME END WHEN ALL FINDS ARE FOUND - AND ASK TO SAVE HS IF ^^
    global highScores
...
def highScore():
    ...
    toSave = font.render("Enter 'Name' and click SAVE to save high score", True, black)
    note = noteFont.render("Note: Saving high score will terminate the game.", True, black)
    save = font.render("Save", True, white)
    pygame.draw.rect(frame, black, [390, 650, 85, 40])
    frame.blit(toSave, (220, 560))
    frame.blit(note, (300, 580))
    frame.blit(save, (410, 660))
...
def showScore():
    font = pygame.font.Font('freesansbold.ttf', 20)
    pscore = font.render('Score: ' + str(player1.score), True, black)
    frame.blit(pscore, (750, 10))
    escore = font.render('Score: ' + str(enemy1.score), True, black)
...
class Enemy(Player):
    ...
    def change_orientation(self):
        ...
        #     change = ['left', 'up', 'down']
        # elif self.orientation == 'left':
        #     change = ['right', 'up', 'down']
        # elif self.orientation == 'up':
        #     change = ['right', 'left', 'down']
        # elif self.orientation == 'down':
        #     change = ['right', 'left', 'up']
...
# Game Loop
def gameLoop():
    global menuOpen, treasuresOpen, highScoresOpen, inPlay, digIt, playerDigs, recentDig, recentItem, recentImage, username, WIDTH, HEIGHT
    global enx, eny, enmovex, enmovey
    ...
    enmovex = 0
    enmovey = 0
    # move_enemy = enemy1.move((enx, eny))
    ...
    while not quitGame:
        ...
        if recentDig == True:
            ...
            # enemy1.showDug(recentItem, recentImage)
